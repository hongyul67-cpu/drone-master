...
var CFG, START=Date.now(), MISS={}, ORDER=[], QUIZ_RESULT=null;
  ...
  shuffle(step.data.map(function(r){return r.a})).forEach(function(a){
    ...
    b.addEventListener("click",function(){
      ...
      if(r.a===a){
        ...
        MISS[step.key]++; say("#m-"+step.key,"no",r.k+"의 설명이 아니다.");
      }
    });
    opts.appendChild(b);
  });
  ...
  shuffle(step.data.slice()).forEach(function(t){
    ...
    b.addEventListener("click",function(){
      if(t===step.data[cur]){
        ...
        else say("#m-"+step.key,"ok","맞다! 다음 순서는?");
      }else{
